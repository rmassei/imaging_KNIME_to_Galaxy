...
from imaging_knime_to_galaxy.evaluation_functions import (
    testing_report,
)
from imaging_knime_to_galaxy.examples import (
...
from imaging_knime_to_galaxy.prompts import (
    build_description_task_prompt,
    build_report_prompt,
    build_summary_prompt,
    build_task_prompt,
)
from imaging_knime_to_galaxy.rag_functions import (
    EMBEDDING_MODEL,
    build_all_docs,
    embed,
    search_store_for_hits,
)
...
def translate_knime_to_galaxy(
    ...
    if EMBEDDING_MODEL == "Qwen/Qwen3-Embedding-4B":
        print(
            "Embedding Model matches the default. "
            "Trying to load pre-computed embeddings ..."
        )
    ...
    # different embedding model
        ...
        print(
            "Embedding Model does not match the default. Computing new embeddings ..."
        )
        vector_store = VectorStore(embed_fn=embed, texts=texts, metadatas=metas)
        vector_store.save(vector_store_path)
    ...
    description_task = build_description_task_prompt(
        knime_nodes_str, workflow_content, summary_answer
    )
    ...
    record_dict = testing_report(
        output_galaxy_workflow_path, knwf_path, "job.yml", example_image_path
    )
