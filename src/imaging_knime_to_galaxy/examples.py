...
def load_translation_examples(yaml_path: str = "data/translation_table.yml") -> list:
    with open(yaml_path, encoding="utf-8") as f:
        ...
        for doc in docs[0]:
            ...
            if knime and galaxy:
                examples.append({"KNIME": knime.strip(), "Galaxy": galaxy.strip()})

    return examples


def build_translation_examples(yaml_path):
    examples_text = """You are a translator that converts KNIME workflow nodes to
...
def build_workflow_examples(yaml_path):
    ...
    Here are some examples of complete KNIME workflows and their
    corresponding Galaxy workflows:
    """
    examples = load_translation_examples(yaml_path=yaml_path)
    if examples:
