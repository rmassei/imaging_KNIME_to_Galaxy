...
from imaging_knime_to_galaxy.rag_functions import build_all_docs, search_store_for_hits
...
def embed_qwen(texts):
    ...
    response = client.embeddings.create(input=texts, model="Qwen/Qwen3-Embedding-4B")
...
def embed_harrier(texts):
    ...
    if _harrier is None:
        _harrier = SentenceTransformer(
            "microsoft/harrier-oss-v1-0.6b", model_kwargs={"dtype": "auto"}
        )
...
def embed_perplexity(texts):
    ...
    if _perplexity is None:
        _perplexity = SentenceTransformer(
            "perplexity-ai/pplx-embed-v1-4B", trust_remote_code=True
        )
...
def evaluate_retrieval(vector_store, benchmark_cases, k=5):
    ...
    for case in benchmark_cases:
        ...
        retrieved_tool_ids = [hit["meta"].get("tool_id") for hit in hits[:k]]
